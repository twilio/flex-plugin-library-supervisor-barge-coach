...
export const enableBargeCoachButtonsUponMonitor = async (flex: typeof Flex, manager: any) => {
  try {
    ...
    flex.Actions.addListener('afterMonitorCall', (payload) => {
      ...
      const supervisorParticipant = payload.task?.conference?.source?.channelParticipants?.find(
        (p: any) =>
      ...
      const agentParticipant = payload.task?.conference?.participants?.find(
        (p: any) => p.participantType === 'worker' && agentWorkerSID === p.workerSid,
      );
